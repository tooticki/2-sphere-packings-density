...
#include <algorithm>
/*
Idea: treat blocks in deque until we have limit number of blocks
Then show which are the connected components we have in these last blocks
They are so-called 'hard' blocks
*/
...
int n_del[2] = {0,0};
deque <Block> blocks;
int BFSlimit = 16777216; //4194304; //1048576 for magi
...
int run_blocks(I ac, I ad, I bc, I bd, I cd){
    ...
    while(blocks.size()>0 && (n_del[0]+n_del[1]+n_del[2] < blocks.size() || blocks.size()>8) ){//&& blocks.size()< 16384){//  blocks.size() <  1048576&&  n_del[0]+n_del[1]+n_del[2] < blocks.size()){
	if (bf){	    
...
I edge_length(I r1,I r2, double EPS){
    double eps =.1; //TODO: just to go faster?? CHANGE
    return I(lower(r1+r2+EPS)+eps, upper(r1+r2+r*2.)-eps);
}

bool neighbours(Block b1, Block b2){ // if one is smaller take it 
    double eps = .0001;
    int counte = 0, countn=0;
    for (int i=0; i<5; i++){
	if (subset(b1.edges[i], b2.edges[i]) || subset(b2.edges[i], b1.edges[i]) || upper(b1.edges[i] - b2.edges[i]) < eps){
	    counte++;
	}
	else{
	    if(lower(b1.edges[i])-upper(b2.edges[i]) < eps || lower(b2.edges[i])-upper(b1.edges[i]) < eps){
		countn++;
	    }
	}
    }
    return (counte>=4 && countn+counte>=5);    
}

Block merged(Block b1, Block b2){ // if one is smaller, take convex rectang hull
    double eps = .0001;
    Block b;
    for (int i=0; i<5; i++){
	if (subset(b1.edges[i], b2.edges[i]) || subset(b2.edges[i], b1.edges[i]) || upper(b1.edges[i] - b2.edges[i]) < eps){	    
	    b.edges[i] = I(min(lower(b1.edges[i]), lower(b2.edges[i])), max(upper(b1.edges[i]), upper(b2.edges[i])));
	}
	else{
	    if(lower(b1.edges[i]) < upper(b2.edges[i])){
		b.edges[i] = I(lower(b1.edges[i]), upper(b2.edges[i]));
	    }
	    else{
		b.edges[i] = I(lower(b2.edges[i]), upper(b1.edges[i]));
	    }
	}
    }
    return b;    
...
void find_connected_components(){
    // in the deque blocks
    // merging ??
    cout << "Blocks initial size: " << blocks.size()<<endl;
    deque <Block> merged_blocks;
    while(blocks.size()>0){
	Block b = blocks.front();
	blocks.pop_front();
	auto ib=blocks.begin();
	while (ib!= blocks.end()){
	    Block b2 = *ib;
	    if (neighbours(b, b2)){
		b = merged(b,b2);		
	...
	merged_blocks.push_front(b);
    }
    cout << "Blocks merged size: " << merged_blocks.size()<<endl;
    for (auto ib=merged_blocks.begin(); ib!= merged_blocks.end(); ++ib){
	Block b2 = *ib;
	cout << "-------------------";
	for (int i=0; i<5; i++){
	    cout << b2.edges[i] << " ";
...
int find_hard_blocks(I rA, I rB, I rC, I rD, int c_type){
    ...
    int limit = 1048576;
    AB = rA+rB;
    I AC = I(lower(rA+rC), upper(rA+rC+.0001)),  AD = I(lower(rA+rD), upper(rA+rD+.0001)); //AC,AD close to tight
    //I AC=edge_length(rA,rC,EPS), AD=edge_length(rA,rD,EPS); TODO uncomment, special testing
    I BC=edge_length(rB,rC,EPS), BD=edge_length(rB,rD,EPS), CD=edge_length(rC,rD,EPS);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 6){
	cout << "> ./program r1 r2 r3 r4 N \n ri in {1,r}, N in {1} 1 contact {21-25} the type of 2 contacts\n";
	return 0;
    }
    cout << "Radii : ";
    for (int i = 1; i < 5; i++){
    ...
    cout << "contact type: " << atoi(argv[5]) << endl;
    auto start = high_resolution_clock::now();
    
    int result = find_hard_blocks(rval(argv[1]), rval(argv[2]), rval(argv[3]), rval(argv[4]), atoi(argv[5]));
    
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<seconds>(stop - start);
    cout << "Execution time: " << duration.count() << " seconds " << endl;
    cout << "In total " << n_del[0]+n_del[1]+n_del[2] << " deleted: " << n_del[0] << " good, " << n_del[1] <<  " no FM"  << endl;
    return  result;
} 
