...
int run_blocks(I ac, I ad, I bc, I bd, I cd){
	...
	if (double(blocks.size()/max(N_blocks,1)) >= 2 || double(N_blocks/(blocks.size())) >= 2 ){
	    ...
	    cout << N_blocks << " blocks;  " << n_del[0]+n_del[1]+n_del[2] << " deleted" << endl;
	}
	
	memcpy(block,  blocks.front(), 5*sizeof(I));	

	blocks.pop();
	AC=block[0]; AD=block[1]; BC=block[2]; BD=block[3]; CD=block[4];
	...
	if(upper(dens)<lower_ud){// good	    
		...
		bb0[i] = block[i];
		bb1[i] = block[i];
		if (width(block[i])>maxw){
		    maxi = i;
		    maxw = width(block[i]);		    
		}
	    }
	    bb0[maxi] = bisect(block[maxi]).first;
	    bb1[maxi] = bisect(block[maxi]).second;
	    blocks.push(bb0);
	    blocks.push(bb1);
	}
...
int main(int argc, char *argv[])
{
    ...
    int result = test_tetrahedra(rval(argv[1]), rval(argv[2]), rval(argv[3]), rval(argv[4]), atoi(argv[5]));
